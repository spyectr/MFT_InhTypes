classdef auxMFT
    methods(Static)
        ...
        function Files=File_Info_MFT(nn,p,flg)
            ...
            % file to save stable states with grouped pops
            filesave_grouped=fullfile(currentfolder,sprintf('%s%s_FP_grouped_data[p%d]_%s.mat',Opt,extra,p,kind));
            ...
            Files.grouped_stable=filesave_grouped;
            Files.Log=log_file;
            Files.m=file_m;
            Files.f=file_f;
            Files.bif=file_bif;
        ...
        function [Risp,flg]=RISP(ni,Params,flg)
            ...
            QUENCH=0;
            if any(strcmp(fieldnames(flg),'cue_mode'))
                if flg.cue_mode
        ...
        function [Sigma_dyn,flg]=SIGMA_DYN(ni,Sigma_dyn,Params,delta_t,flg)         % compute Sigma_dyn^2[1,...,4]
            ...
            taus=Params.Tau;
            if any(strcmp(fieldnames(flg),'syn_dyn'))
                if flg.syn_dyn
                    taus=Params.Tausyn;
                end
            end
...
%             Jminus = 1.-gam*f*(Jplus-1.);
            ...
            if (nn == 4)
                Params_out=auxMFT.In_Param_4pop_InhType([theta_e theta_i],paramsfile,flg);
            else
                Params_out=aux.In_Parameters_InhType([theta_e theta_i],paramsfile,flg);  %% NO Overlap
                if flg.grouped>0
                    Params_out=aux.In_Parameters_InhType_grouped([theta_e theta_i],paramsfile,flg);  %% NO Overlap
                end
            end
        end
...
%             % erdos renyi
%             B(1,1) = tau_e*Cee*(1-p_ee).*x*Jee^2;
%             B(1,2:n) = tau_e*Cei.*(1-p_ei).*Jei.^2;
%             B(2:n,1) = x*tau_i'.*Cie.*(1-p_ie).*Jie.^2;
%             B(2:n,2:n) = repmat(tau_i',1,3).*Cii.*(1-p_ii).*Jii.^2;
            ...
            Sigma_ext=zeros(1,n);
%             if any(strcmp(fieldnames(Network),'stim'))
...
%             end
            ...
            if any(strcmp(fieldnames(flg),'paramsfile'))
        ...
        function [theta_out,flg]=fun_Fix_Threshold_InhType(ni,params,flg)
...
%             end
            ...
            params=auxMFT.In_Param_4pop_InhType([theta_e, theta_i],paramsfile,flg);
            % params.A
            % params.B
            % Risp=auxMFT.RISP(ni,params);
            [Mu,flg]=auxMFT.MU(ni,params,flg); % RUN FUNCTION
            [Sigma2,flg]=auxMFT.SIGMA2(ni,params,flg); % RUN FUNCTION
            if flg.return_value == 0
