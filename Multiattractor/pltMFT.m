classdef pltMFT
    methods(Static)
        ...
        function PlotStableStates(p,Jstop,NumFig,flg)
            ...
            figure(NumFig); clf; h=[];Leg=[];hold on;      NumFig=NumFig+1;                        fntsz=15;
...
%                 h(cnt_crit)=plot(Jplus, rates(:,1),plotstyle,'color',[0 0 0]+(2*(N-cnt_crit))/(2*N+8),'markersize',12);
                h(cnt_crit)=plot(Jplus, rates(:,1),plotstyle,'color',[0 0 0]+(2*(N-cnt_crit))/(2*N+8),'linewidth',2);
                text(Jplus(end), rates(end,1),num2str(n),'color','r');
                plot(Jplus, rates(:,p),plotstyle,'color',[(2*(N-cnt_crit))/(2*N+8)*[1,1],0],'linewidth',2);
                text(Jplus(end), rates(end,p),num2str(n),'color','r');
                Leg{cnt_crit}=num2str(n);
                subplot(2,2,2);hold on; 
%                 h(cnt_crit)=plot(Jplus, rates(:,1),plotstyle,'color',[0 0 0]+(2*(N-cnt_crit))/(2*N+8),'markersize',12);
                plot(Jplus, rates(:,end-2),plotstyle,'color',[(2*(N-cnt_crit))/(2*N+8),0,0],'linewidth',2);
                text(Jplus(end), rates(end,end-2),num2str(n),'color','r');
                subplot(2,2,3);hold on; 
                plot(Jplus, rates(:,end-1),plotstyle,'color',[0 (2*(N-cnt_crit))/(2*N+8),0],'linewidth',2);
                text(Jplus(end), rates(end,end-1),num2str(n),'color','r');
                subplot(2,2,4);hold on; 
                plot(Jplus, rates(:,end),plotstyle,'color',[0 (2*(N-cnt_crit))/(2*N+8)*[1,1]],'linewidth',2);
                text(Jplus(end), rates(end,end),num2str(n),'color','r');
            end
            subplot(2,2,1); hold on; 
...
%                     plot(Jplus(1:indEnd), rates(1:indEnd,1),plotstyle,'color','b','markersize',8);
                    subplot(2,2,1); h(cnt_crit+1)=plot(Jplus(1:indEnd), rates(1:indEnd,1),plotstyle,'color','b','linewidth',2);
                    for iInh=1:3; subplot(2,2,1+iInh); 
                        plot(Jplus(1:indEnd), rates(1:indEnd,end-3+iInh),plotstyle,'color','b','linewidth',2); 
                        text(Jplus(indEnd), rates(indEnd,end-3+iInh),num2str(p),'color','b');
                    end
                    subplot(2,2,1); Leg{cnt_crit+1}=num2str(p);
                    hold on;
                    line([Jplus(indEnd) Jplus(indEnd)],[0 YLimRate],'linewidth',1.5,'linestyle','-.','color','b');
                end
            end
            titl=[{'Pyr'},InhLab];
            for i=1:4; subplot(2,2,i); xlab='J+'; ylab='Firing rate [spks/s]'; xlim([Jzero Jstop]); utils.figset(gca,xlab,ylab,titl{i},fntsz); end
%             legend(h,Leg);
