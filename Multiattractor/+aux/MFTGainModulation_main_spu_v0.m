...
% function MFTGainModulation_main_spu(options_main)
...
if any(strcmp(fieldnames(Network),'ThresholdOpt'))
    if Network.ThresholdOpt
        [theta,flg]=auxMFT.fun_Fix_Threshold_InhType(Ni,params,flg);
...
Params=auxMFT.In_Param_4pop_InhType(theta,flg.paramsfile,flg);
...
if (~res)  
    ...
    [~,unind]=max(Real_part);
    unstabledir=0.1*EigV(:,unind)';
    [Ni,flg]=auxMFT.goAB_simul(Ni+1*unstabledir,0,Params,flg); % 0 := no max number of steps.
    [res, Real_part, Imag_part, EigV,flg]=auxMFT.Test_Stability(Ni,Params,flg);
    if (res) 
...
%     for nn=6:8%p+4
nn=6;         n_hiMax=20; n_hiMax=min([n_hiMax,p-1]);
%             flg.grouped=n_hi;
...
%         [Ni,flg]=auxMFT.mnewt(flg.NTRIAL,Ni,Params,flg);
        for n_hi= 1:n_hiMax
            flg.grouped=n_hi;
        Params=auxMFT.Learn(Params,flg);
            fprintf(' Running with %d hi grouped sel populations...\n',n_hi);
...
%     end
    ...
    save(Params.Files.grouped_stable,'StableStates','UnstableStates');
end
...
% CHECK ALL mode: -------------------------------------------------------
% -----------------------------------------------------------------------
% this part checks that the attractors in the reduced system are also attractors of the full system
if (any(bitand(flg.flags,flg.RATE_VS_J)) && flg.all_mode==1)% && flg.stim_mode==0
    flg.nivsj_mode=1;
    flg.contr=0;
    flg.grouped=0;
...
%     [Ni,flg]=auxMFT.mnewt(flg.NTRIAL,Ni,Params,flg);
    ...
    grouped_all=load(Params.Files.grouped_stable,'StableStates','UnstableStates');
    for n_hi= 1:n_hiMax
        fprintf(' Running with %d hi populations...\n',n_hi);
        if n_hi<p
            rates_grouped=grouped_all.StableStates(n_hi);
        else
            filespont=auxMFT.File_Info_MFT(5,p,flg);
            rates_grouped=load(filespont.FP,'Jplus_store','rates');
            rates_grouped.Jplus=rates_grouped.Jplus_store;
        end
        [StableStates(n_hi), UnstableStates(n_hi),flg]=aux.CHECK_STABILITY_ALL(n_hi,rates_grouped,Jstop,Params,Opt,flg);
        if (flg.return_value == 0)
            fprintf('\n>>> Error in routine RATE_VS_Jfun_ALL() called by main()\n');
        end
    end
