...
function [res_stable res_unstable flg]=SPURIOUS(ni,Jstop,Params,Opt,flg)
% ni=Ni
...
load(paramsfile);
% nn=numel(ni); nn_sel = nn-5;

TOL=0.0001; % minimum difference between ni(1)>ni(2) firing rates to accept sel
% if 1%nn<50
%     MAX_DATA=nn;
% else
%     MAX_DATA=50;
...
for Jplus=JplusArray
    ...
    if (flg.nivsj_mode == 1)
        % NO OVERLAP CASE:
        if 1%~isempty(strfind(Opt,'Small2')) || ~isempty(strfind(Opt,'InhTypes'))
                Nkick=20;
...
%                 end
            elseif any(rates_old(cnt,:))
                if flg.stim_mode==0 || (flg.stim_mode==1 && nn>5)
                    fprintf('Loading rates from rates_old...\n');
                    ni=rates_old(cnt,:);
                    ni(1)=Nkick;
%                     ni(end-1)=rates_old(cnt,end-1);
%                     ni(end)=rates_old(cnt,end);
%                     if nn==p % use spontaneous
%                         ni=zeros(1,p+4);
...
%                 end
%                 if n_hi==2
%                     ni(1:n_hi)=30; ni(n_hi+1:nn-4)=3; ni(nn-3)=2; ni(nn-2:nn)=[12, 30,5];
%                     ni
%                 end
...
% %                 skip_mode=1;
    ...
    if ~skip_mode
        ...
        if res; fprintf('Stable fixed point.\n'); end
        ...
        fprintf('\n');
        % check this is selective activity and not spontaneous
        result=(ni(1)>ni(2)+TOL);
        fprintf('    fixed point:');
        for j=1:nn
            fprintf(' %0.03g ',ni(j));
        end
        ...
        if result && res
            ...
            fprintf('\n --- Fixed point with %d grouped Active Clusters ---> stored...\n',flg.grouped);
