...
if any(strcmp(fieldnames(Network),'cuemode'))
    ...
    switch Network.cuemode
        case 'inh_mean'
            Mu_ext(end-2:end)=Mu_ext(end-2:end)*(1+cue_value);
        case 'exc_mean'
            Mu_ext(1:end-4)=Mu_ext(1:end-4)*(1+cue_value);
        case 'exc_gaussian'
            %--------------
            % GAUSSIAN CUE
            %--------------
        ...
        case 'inh_gaussian'
            ...
            % function to be integrated later for quenched noise computation with
            % gaussian envelope
            %%% potential problem: must define a function for each
            %%% population because of the tau_i.*Jie_ext vector inferering with the Z!!!!
            Mu_extZ  = @(Z)Cext*ni_ext_i*tau_i.*Jie_ext*cue_value*Z.*heaviside(1+cue_value*Z);
            Params.Mu_extZ=Mu_extZ;
            if nn>=5
                Params.quench_pops=nn-2:nn;
            end
        case 'Chance'
            for i=1:nn-1
                Sigma_ext(i)  = (1.+delta)*tau_e*Cext*ni_ext_e*Jee_ext^2*cue_value;
            end
            Sigma_ext(INIB) = (1.+delta)*Cext*tau_i.*ni_ext_i.*Jie_ext.^2*cue_value;     
    end
%         pcue=Network.pcue;
%         mu_cue=0; 
%         sigma_cue=0;
