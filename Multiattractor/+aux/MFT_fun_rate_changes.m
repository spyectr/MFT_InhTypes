...
for i_c=1:ncueset
    ...
    %     p=params.p;
        ...
        % find states
        Ind0=find(cell2mat(arrayfun(@(x)~isempty(x(:).Jplus),AllData.StableStates,'uniformoutput',false))); % states with Ind active clusters
        % Select attractors for current Jplus
        % only keep selective and spontaneous for simplicity
        Ind=Ind0(Ind0==p | Ind0==1);
        for n=Ind
...
Legends=cell(6,1);
Legends{1}='';
Legends(2:6)={'bg (Exc)','PV','SST','VIP','mean Inh'};
...
for iJ=1:numel(Jcases)
    ...
    for ic=unique(Clust)'
        if sum(Clust==ic)>1
            ...
            y=rate(Clust==ic,:); Legends{1}=sprintf('ActClust=%d',ic);
            x=cueval(Clust==ic)';
            h(1)=plot(x,y(:,1),'linewidth',2); hold on;
            h(2:5)=plot(x,y(:,nn-3:nn),'linewidth',2); hold on;
...
%             exctot=exctot-repmat(exctot(zerocue),1,numel(zerocue));
            y=y-yref;
            h(1)=plot(x,y(:,1),'linewidth',2); hold on; Legends{1}=sprintf('ActClust=%d',ic);
            h(2:5)=plot(x,y(:,nn-3:nn),'linewidth',2); hold on;
            h=[h,plot(x,inhtot,':','linewidth',2)]; hold on;
...
for iJ=2%1:numel(Jcases)
    ...
    for ic=unique(Clust)'
        if sum(Clust==ic)>1
            ...
            Legends{1}=sprintf('ActClust=%d',ic);
            h1=plot(x,inhCurr(:,[1,nn-4:nn])','-','linewidth',2); hold on;
            plot(0,0,'k'); hold on;
            h2=plot(x,excCurr(:,[1,nn-4:nn])',':','linewidth',2); hold on;
            % plot sum of all Inh pops
%             inhtot=inh_frac*rate(Clust==ic,end-2:end)';
%             h=[h;plot(x,inhtot,':','linewidth',2)]; hold on;
...
%             if cnt==1; 
                legend([h1;h2],[Legends;Legends],'location','northeast','fontsize',8);
%             end
%             ylim(YLIM);
        end
    end
...
%             set(gca,'Xtick',1:size(x,1), 'XTickLabel',poplabel,'Ytick',1:size(x,2), 'YTickLabel', poplabel);
            tit=sprintf('J+=%0.03g,ActCl=%d,cue=%0.02g',Jvalue.(Jcases{iJ}),Clust(cnt),cueval(cnt));
%             title(tit);
            utils.figset(gca,'','\Delta SST to VIP [Hz]',tit,15);
%         end
    end
%             p=utils.mtit('rows=out,cols=in');
    filesave=Files.paperenergy;
    saveas(gcf,[filesave '[Jp' sprintf('%0.03g',Jvalue.(Jcases{iJ})) ']_LinRespSST.pdf'],'pdf');
end
