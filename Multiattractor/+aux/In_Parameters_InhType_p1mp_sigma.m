...
for i=1:nn_sel
    % sel -> itself:
    B(i,i) = Jee*Jee*tau_e*f*Cee*(1-p_ee)*x*Jplus*Jplus;
    if HiIntraCluster
        B(i,i) = B(i,i)*(peein/peeout)*(1-peein/peeout);
    end
    for j=1:nn
        % sel(j) -> sel(i):
        if (j~=i && j<=nn_sel)  
          B(i,j) = Jee*Jee*tau_e*f*Cee*(1-p_ee)*x*Jminus*Jminus;
        end
        % all other sel -> sel:
        if (j==OTHER_SEL)  
          B(i,j) = Jee*Jee*tau_e*f*(p-nn_sel)*Cee*(1-p_ee)*x*Jminus*Jminus;
        end
        % bg -> sel:
        if (j==BACK_EXC)   
          B(i,j) = Jee*Jee*tau_e*(1-p*f)*Cee*(1-p_ee)*x*Jminus*Jminus;
        end
        % inib -> sel:
        if any(j==INIB)       
          B(i,j) = tau_e*Cei(j==INIB)*(1-p_ei(j==INIB))*Jei(j==INIB)*Jei(j==INIB);
        end
    end
end
...
B(i,i)        = tau_e*x*f*Cee*(1-p_ee)*Temp*Jee*Jee;
if HiIntraCluster
    Temp1=((p-nn_sel-1)*Jminus*Jminus+(peein/peeout)*Jplus*Jplus);
    B(i,i) = B(i,i)*Temp1/Temp;
end
% bg  -> all other sel collected: 
B(i,BACK_EXC) = Jee*Jee*tau_e*(1.-f*p)*Cee*(1-p_ee)*x*Jminus*Jminus;
% inib  -> all other sel collected:
B(i,INIB)     = tau_e*Cei.*(1-p_ei).*Jei.*Jei;
...
for j=1:nn_sel
    % sel ->  bg: 
    B(i,j) = tau_e*f*Cee*(1-p_ee)*x*Jminus*Jee*Jminus*Jee;
end
% all other sel (collected) ->  bg:
B(i,OTHER_SEL) = tau_e*f*(p-nn_sel)*Cee*(1-p_ee)*x*Jminus*Jee*Jminus*Jee;
% bg  ->  itself:
B(i,i)         = tau_e*(1-p*f)*Cee*(1-p_ee)*x*Jee*Jee*(1+Jcor*Jcor);
% inib  ->  bg:
B(i,INIB)      = tau_e*Cei.*(1-p_ei).*Jei.*Jei;
...
for j=1:nn_sel
    % sel -> inib:
    B(i,j) = f*x*tau_i'.*Cie.*(1-p_ie).*Jie.*Jie; 
end
% all other sel -> inib: 
B(i,OTHER_SEL) = f*(p-nn_sel)*x*tau_i'.*Cie.*(1-p_ie).*Jie.*Jie;
% bg -> inib:
B(i,BACK_EXC)  = (1-p*f)*x*tau_i'.*Cie.*(1-p_ie).*Jie.*Jie;
% inib -> itself:
B(i,i)         = Cii.*(1-p_ii)*repmat(tau_i',1,3).*Jii.*Jii;
if (delta ~= 0) 
    for i=1:nn
        for j=1:nn 
            B(i,j) =B(i,j) *( 1.+delta);
