...
function Params=In_Parameters_InhType_grouped(theta,paramsfile,flg)
...
for i=1
%     % sel -> itself:
    A(i,i) = tau_e*f*Cee*x*(Jplus+(nn_sel_grouped-1)*Jminus)*Jee;
    if HiIntraCluster
...
%         end
        ...
        %       % all other sel -> sel:
        if (j==OTHER_SEL)  
          A(i,j) = tau_e*f*(p-nn_sel_grouped)*Cee*x*Jminus*Jee;
        end
...
for j=1
    ...
    A(i,j) = tau_e*f*(nn_sel_grouped)*Cee*x*Jminus*Jee;
end
% all other sel collected -> itself:
Temp=((p-nn_sel_grouped-1)*Jminus+Jplus);
A(i,i)        = tau_e*x*f*Cee*Temp*Jee;
if HiIntraCluster
    Temp1=((p-nn_sel_grouped-1)*Jminus+(peein/peeout)*Jplus);
...
for j=1
    % sel ->  bg: 
    A(i,j) = tau_e*f*(nn_sel_grouped)*Cee*x*Jsel_to_bg*Jee;
end
% all other sel (collected) ->  bg:
A(i,OTHER_SEL) = tau_e*f*(p-nn_sel_grouped)*Cee*x*Jsel_to_bg*Jee;
% bg  ->  itself:
...
for j=1
    % sel -> inib:
    A(i,j) = f*(nn_sel_grouped)*x*tau_i'.*Cie.*Jie;
end
% all other sel -> inib:

A(i,OTHER_SEL)= f*(p-nn_sel_grouped)*x*tau_i'.*Cie.*Jie;
% bg -> inib:
...
for i=1
    % sel -> itself:
    B(i,i) = Jee*Jee*tau_e*f*Cee*x*(Jplus^2+(nn_sel_grouped-1)*Jminus^2);
    if HiIntraCluster
        B(i,i) = B(i,i)*(peein/peeout);
    end
    for j=1:nn
        % sel(j) -> sel(i):
%         if (j~=i && j<=nn_sel_grouped)  
%           B(i,j) = Jee*Jee*tau_e*f*Cee*x*Jminus*Jminus;
%         end
        % all other sel -> sel:
        if (j==OTHER_SEL)  
          B(i,j) = Jee*Jee*tau_e*f*(p-nn_sel_grouped)*Cee*x*Jminus*Jminus;
        end
        % bg -> sel:
        if (j==BACK_EXC)   
...
for j=1
    % sel -> all other sel collected: 
    B(i,j) = Jee*Jee*tau_e*f*nn_sel_grouped*Cee*x*Jminus*Jminus;
end
% all other sel collected -> itself:
Temp=((p-nn_sel_grouped-1)*Jminus*Jminus+Jplus*Jplus);
B(i,i)        = tau_e*x*f*Cee*Temp*Jee*Jee;
if HiIntraCluster
    Temp1=((p-nn_sel_grouped-1)*Jminus*Jminus+(peein/peeout)*Jplus*Jplus);
...
for j=1
    % sel ->  bg: 
    B(i,j) = tau_e*f*nn_sel_grouped*Cee*x*Jminus*Jee*Jminus*Jee;
end
% all other sel (collected) ->  bg:
B(i,OTHER_SEL) = tau_e*f*(p-nn_sel_grouped)*Cee*x*Jminus*Jee*Jminus*Jee;
% bg  ->  itself:
...
for j=1
    % sel -> inib:
    B(i,j) = f*(nn_sel_grouped)*x*tau_i'.*Cie.*Jie.*Jie; 
end
% all other sel -> inib: 
B(i,OTHER_SEL) = f*(p-nn_sel_grouped)*x*tau_i'.*Cie.*Jie.*Jie;
% bg -> inib:
...
if any(strcmp(fieldnames(Network),'syn'))
...
if flg.cue_mode
    ...
    for i=1:numel(cue_option)
        ...
        elseif strcmp(cue_stat{i},'gaussian')
            Params.quench_pops=[Params.quench_pops indpop];
            ...
            for ipop=1:numel(indpop)
                Mu_extZ{indpop(ipop)}=Mu_extZ0;
            end
