...
function [res_stable res_unstable flg]=CHECK_STABILITY_ALL(n_hi,rates_grouped,Jstop,Params,Opt,flg)
% ni=Ni
...
Jplus = Jzero;
% create JplusArray
JplusArray=rates_grouped.Jplus;
% JplusArray=[];
% while Jplus<=Jstop
...
%         end
        ni=NaN(1,nn);
        ni_grouped=rates_grouped.rates(cnt,:);
        ni(1:n_hi)=ni_grouped(1);
        ni(n_hi+1:p)=ni_grouped(2);
        ni(nn-3:nn)=ni_grouped(end-3:end);
        % Check fixed point:
