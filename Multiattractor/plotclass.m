        ...
        function fun_PlotTrial(data,firings,Params)
            ...
            if plot_raster
                if any(any(firings))
                    ...
                    option='poporder';
                    fieldNames={'fieldNames','N_e','N_i','Ne_plot','Ni_plot','NcE','XLIM','option','p','clustermatrix','colors'};
            ...
            if plot_currents
                ...
                subplot(numel(ind_plot),1,1);
%                 PlotMembrane(Sim,vplot/Jee-100,firings,ind_plot,VTh/Jee-100,colors);
                PlotMembrane(Sim,vplot,firings,ind_plot,VTh,colors);
                xlim(XLIM);
                saveas(gcf,[filesave '_membrane.pdf'],'pdf');
                
                figure(numfig); clf;
                set(gcf, 'Position',  [100, 100, 800, 800])
                numfig=numfig+1;
                nplot=numel(ind_plot)+1;
                subplot(nplot,1,1);
...
%                 PSCPlot(Sim,Ext,iIplot*Jee,iEplot*Jee,iExtplot*Jee,VTh*Jee./(tau),ind_plot);
                PSCPlot(Sim,Ext,iIplot,iEplot,iExtplot,VTh./(tau),ind_plot,colors);
                xlim(XLIM);
                % SAVE PLOT
            ...
            if plot_rates
                if p>1 && any(any(firings))
                    ...
                    % A. Cluster PSTH
                    subplot(2,1,1)
                    [rate, x_bins]=PlotPopRates(bins,firings,N_e,N_i,NcE,colors);
                    xlim(XLIM);
            ...
            function PlotAllNeurons(Ext,firings,IN)
                ...
                %
                cusumNcE=[0 cumsum(NcE)'];
                hh(1)=plot(firings(firings(:,2)<=Ne_plot,1),firings(firings(:,2)<=Ne_plot,2),'.','color',colors{1},'markersize',1);
                cumN_i=[0 cumsum(N_i)];
                for i_n=1:numel(Ni_plot)
                    hh(1+i_n)=plot(firings(firings(:,2)>N_e+cumN_i(i_n) & firings(:,2)<N_e+cumN_i(i_n)+Ni_plot(i_n),1),...
                        firings(firings(:,2)>N_e+cumN_i(i_n) & firings(:,2)<N_e+cumN_i(i_n)+Ni_plot(i_n),2)-N_e+Ne_plot,...
                        '.','color',colors{i_n+1},'markersize',1);
                end
                if any(strcmp(fieldnames(Ext),'stim'))
                ...
                line([0 0],[0 Ne_plot+sum(Ni_plot)],'color','b');
                if numel(Ni_plot)==1; labpop={'E','I'}; else; labpop={'E','PV','SST','VIP'}; end
                legend(hh,labpop,'location','northeast');
                hold on;
                ylim([0 Ne_plot+sum(Ni_plot)]);
                xlab='Time (s)';
                ylab='Neurons';
            ...
            function PSCPlot(Sim,Ext,iIplot,iEplot,iExtplot,VThbyTau,ind_plot,colors)
                binwidth=35;
                stimcolors={'m',[0.5,0.5,0.5]}; 
                nplot=numel(ind_plot);
                dt=Sim.dt_step;
%                 hE=[]; hI=[]; hext=[]; htot=[]; hth=[];
...
%                 ExcInh={'Exc','Inh'};
                ...
                % legend
                Legplot={'i_E'};
                for i=1:size(iIplot,3); Legplot=[Legplot {['i_' ExcInh{i+1}]}]; end
                Legplot1=[Legplot, {'i_{ext}','i_{tot}','V_{th}/\tau'}];
                for j=1:nplot
                    subplot(nplot,1,j);  hstim=[];
                    hold on;
                    % E current
                    
                    [tout iESmooth] = utils.gaussfilt(t_plot,iEplot(j,:),binwidth,0);
                    plot(tout, iESmooth,'color',colors{1},'linewidth',1); % EPSC
                    % I currents
                    for i_i=1:size(iIplot,3)
                        [tout iISmooth] = utils.gaussfilt(t_plot,iIplot(j,:,i_i),binwidth,0);
                        plot(tout, iISmooth,'color',colors{i_i+1},'linewidth',1); % IPSC
                    end
                    % Ext
                    plot(tout, iExtplot(j,:),'color',[.5,.5,.5],'linewidth',1); % EPSC
                    % Total
                    itot=iEplot(j,:)+sum(iIplot(j,:,:),3)+iExtplot(j,:);
                    ...
                    plot(tout,itotSmooth,'color','k','linewidth',1); % membrane potentials
                    % Thresholds/tau
                    plot([tout(1) t_plot(end)],ones(1,2)*VThbyTau(ind_plot(j)),'color',[.5,.5,.5],'linewidth',1,'linestyle',':');
                    grid on
                    if any(strcmp(fieldnames(Ext),'stim'))
                        for n=1:numel(Ext.stim)
                            time_bin=[Ext.stim(n).interval(1) Ext.stim(n).interval(2)];
                            ...
                            line(time_bin([1,end]),(1+0.1*n)*lower([1,end]),'color',stimcolors{n},'linewidth',5);
                            Legplot2=[Legplot1,Ext.stim(n).name];
                        end
                    else; Legplot2=Legplot1;
                    end
                    xlab='Time (s)';
                    ylab='mV/s';
                    tt=sprintf('PSC(%s)',ExcInh{j});
                    fntsz=10;
                    utils.figset(gca,xlab,ylab,tt,fntsz);
                    ind_lim=find(t_plot>Sim.t_Start+0.1 & t_plot<Sim.t_End-0.1);
                    if j==1; legend(Legplot2); end
%                     xlim([t_plot(1)+0.2 t_plot(end)]);
                    xlim([t_plot(ind_lim(1)) t_plot(ind_lim(end))]);
            ...
            function PlotMembrane(Sim,vplot,firings,ind_plot,VTh,colors)
                nplot=numel(ind_plot);
                leglabels=[]; poplabel={'E','PV','SST','VIP'};
                dt=Sim.dt_step;
                hh=[]; %cnt=0;
                ...
                for j=1:nplot
                    ...
                    leglabels=[leglabels,poplabel(j)];
                    hold on
                    if any(any(firings))
            ...
            function [rate, x_bins]=PlotPopRates(bins,firings,N_e,N_i,NcE,colors)
                MinRate=10;
                ...
                for clu=1:numel(NcE)
                    ...
                    hh(1)=plot(x_bins-diff(x_bins(1:2))/2,rate(clu,:),'color',cols{1},'linewidth',1.5);
                    hold on;
                end
                % unclustered subpop
                if N_e-cusumNcE(end)>0 % if there's an unclustered population
                    ...
                    hh(2)=plot(x_bins-diff(x_bins(1:2))/2,rate(clu+1,:),'color',cols{2},'linewidth',1.5,'linestyle','--');
                end
                % inhibitory neurons
                cumN_i=[0 cumsum(N_i)];
                for i_n=1:numel(N_i)
                    ind_inh=(firings(:,2)>N_e+cumN_i(i_n) & firings(:,2)<=N_e+cumN_i(i_n+1));
                    spikes=firings(ind_inh,1);
                    x=histc(spikes,bins);
                    [tout, z]=utils.gaussfilt(bins,x,binwidth,0);
                    x_bins=tout;
                    rate=[rate; z'/((N_i(i_n))*BinSize)];
                    hh(2+i_n)=plot(x_bins-diff(x_bins(1:2))/2,rate(clu+1+i_n,:),'color',colors{i_n+1},'linewidth',2,'linestyle',':');
                end
                
                line([x_bins(1)-diff(x_bins(1:2))/2 x_bins(end)],[MinRate MinRate],'linestyle','--','color','k');
                if numel(N_i)==1; labpop={'E','E_{bg}','I'}; else; labpop={'E','E_{bg}','PV','SST','VIP'}; end
                legend(hh,labpop,'location','northeast');
                hold on;
                xlim([bins(1) bins(end)]);
                xlab='Time [s]';
